########################################################################
# Written by Sai Chaitaya Tadepalli , based on Nashwan Sabti - 2021
# Please cite 2110.XXXX and 2110.XXXX when using this likelihood
########################################################################
########################################################################
# Written for Cobaya sampler on Oct 5, 2025
# Authors: Siu Cheung Lam, Sai Chaitanya Tadepalli (speed-ups by vectorization/caching)
# Further modified: Combined HST + JWST support with AP corrections and
#                   asymmetric-error likelihood (sigma_up, sigma_lo).
########################################################################

####################################################################
# Added z-dependence for alphastar
####################################################################
...
class UVLF_ST(Likelihood):
    ...
    def initialize(self):
        ...
        self.k_max = 30.0
        self.kSpace = np.exp(np.linspace(np.log(1e-4), np.log(self.k_max), 1000))
        ...
        self.Mhalos = np.geomspace(1e8, 1e14, 1000)   # changed from 1e8 to 2e8
        self.wM = self._trapz_weights(self.Mhalos)   # trapezoid weights over M

        # --- Force large numeric arrays to single precision for speed/memory ---
        self.kSpace = self.kSpace.astype(np.float32)
        self.Mhalos = self.Mhalos.astype(np.float32)
        self.wM     = self.wM.astype(np.float32)
        # ---- Load UVLF data ----
        base = '/N/slate/saictade/MCMC_runs/projects/WDM_CDI_UVLF'

        # HST (columns: z, MUV, dM, phi, sigma_up, sigma_lo)
        self.UVLF_HST = np.loadtxt(base + '/final_runs/UVLF_HST.txt')
        minError = 0.2  # 0.2 if you want a floor
        ...
        # JWST (Donnan+ 2024)
        try:
            self.UVLF_JWST = np.loadtxt(base + '/final_runs/UVLF_JWST.txt')
        ...
        betadata = np.loadtxt(base + "/data/Beta_parameters.txt", unpack=True)
        ...
        # ---- Parse constants ----
        self.parse_assign(base + '/Likelihoods/UVLF_HST_ST_model2.data', 'UVLF_HST_ST_model2')
        ...
        self._jwst_dz_half = {9.0: 0.5, 10.0: 0.5, 11.0: 0.5, 12.5: 1.0}   # no 14.5
    ...
    def sigma_sharpk_all(self, R_all, Pkz, c_edge=1.0):
        ...
        # linear interpolation is much cheaper than constructing a spline
        sig2 = np.interp(lnkc, lnk, I, left=I[0], right=I[-1])
        return np.sqrt(np.maximum(sig2, 0.0))

    def HMF_all(self, z, Pk_interp, An_sharpk, qn_sharpk, cn_sharpk, mncdm):
        # cosmology pieces
        ...
        # ultra-fast n_eff using two preselected grid points
        p1 = max(Pkz[self._i1], 1e-300)  # at 1/Mpc
        p2 = max(Pkz[self._i2], 1e-300)  # at 15 /Mpc
        neff = np.log(p2 / p1) / self._lnk_ratio        
        
        lnM = np.log(self.Mhalos)

        # tophat σ via cached window (no trig/outer per z)
        sigma_th = self._sigma_from_cached_W2(Pkz)
        # sig_interp1 = PchipInterpolator(lnM, sigma_th)  # or np.interp for speed
        
        # Derivatives: D = d ln σ^{-1} / d ln M = - d ln σ / d ln M
        ...
        f_th  = f_ST(nu_th, self.AST * float(An_sharpk), self.aST, self.pST)      ##### arent we missing a factor of 1.13-1.16 for tophat too ?
        hmf_th = (rhoM / (self.Mhalos**2)) * f_th * D_th   # <-- dn/dM

        # ---- choose regime by n_eff ----
        # if (neff < -2.75) and (Mcut < self.Mhalos[-1]):   # THIS WAS WRONG!!!!
        if (neff < -2.75) and (Mcut > self.Mhalos[0]):
            # Only compute sharp-k if we actually need it
            sigma_sh = self.sigma_sharpk_all(self._R_base_cached, Pkz, c_edge=cn_sharpk)
            D_sh     = -np.gradient(np.log(sigma_sh), lnM, edge_order=2)
            nu_sh    = self.deltaST / sigma_sh
            f_sh     = f_ST(nu_sh, self.AST * float(An_sharpk), self.aST * float(qn_sharpk), self.pST)
            hmf_sh   = (rhoM / (self.Mhalos**2)) * f_sh * D_sh

            # mask_lo = (self.Mhalos < Mcut)
            # hmf = np.where(mask_lo, hmf_sh, hmf_th)
            x = (np.log10(self.Mhalos) - np.log10(Mcut)) / 0.3
            w = 0.5*(1 - np.tanh(x))     # ~1 below Mcut, ~0 above
            hmf = w*hmf_sh + (1-w)*hmf_th        
            # sig_interp2 = PchipInterpolator(lnM, sigma_sh)  # or np.interp for speed

        else:
            hmf = hmf_th
            # sig_interp2 = PchipInterpolator(lnM, sigma_th)  # or np.interp for speed


        return hmf.astype(np.float32) 
    ...
    def MUV_from_Mh_vec(z, Mh, Hubble, alphastar, betastar, epsilonstar_slope, epsilonstar_icept, Mc_slope, Mc_icept
        ...
        if toggle==0:
            ...
            alphastar = alphastar + np.where(z <= 8.5, 0.0 , alphastar_slope * (z - 8.5))
    ...
    def logp(self, _derived=None, **params):
        ...
        if params['toggle']==0:
            ...
            alphastar = params['alphastar'] + np.where(self.zs_all <= 8.5, 0.0 , params['alphastar_slope'] * (self.zs_all - 8.5))
        ...
        for z in self.zs_all:
            ...
            HMFs = self.HMF_all(z, Pk_interp, params['Anorm'], params['qnorm'], params['cnorm'], params['m_ncdm'])
            ...
            sigma_MUV_this = sigma_MUV_hst if (z < 9.0) else sigma_MUV_jwst
